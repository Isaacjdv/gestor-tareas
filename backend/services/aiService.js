...
/* ================================
   ...
   (extrae comandos en formato JSON)
   ================================ */
...
/* ====================================================
   FUNCIÓN 2: El Conversador (string o array de mensajes)
   ==================================================== */
exports.generateConversationalResponse = async (historyOrMessage, userName, userData = {}) => {
  const folders = Array.isArray(userData.folders) ? userData.folders : [];
  const files = Array.isArray(userData.files) ? userData.files : [];
  ...
  const systemInstruction = `
        ...
        DATOS DEL USUARIO:
        Carpetas: ${folders.map(f => f.nombre).join(', ') || 'ninguna'}.
        Archivos Recientes: ${files.slice(0, 5).map(f => `${f.nombre_original} (Estado: ${f.status || 'pending'})`).join('; ') || 'ninguno'}.
...
/* =======================================
   FUNCIÓN AUXILIAR: Buscar imagen (Unsplash)
   ======================================= */
...
/* ==========================================================
   FUNCIÓN 3: Generador de Contenido para PDF (Restaurada)
   Estructura: Intro, Secciones, Conclusión, Bibliografía
   ========================================================== */
exports.generatePdfContent = async (topic, userName) => {
  ...
  // PROMPT 1: Estructura + imageQuery
  const structurePrompt = `
        ...
        1.1 Eviata usar caracteres raros como "*# "
        ...
        5. "imageQuery": Una frase corta, fotorrealista, y en INGLÉS, para buscar la imagen de portada en Unsplash. Debe ser específica del tema.

        REGLAS DE imageQuery:
        - Debe ser fotorrealista (ej: "historical world war 2 photo", "D-Day soldiers").
        - Si es un dibujo animado (ej: Ben 10), pide "Ben 10 cartoon character" o "Ben 10 realistic art".
        - NO debe ser abstracta (evitar "peace" o "war concept").
        - NO debe incluir texto.
  ...
  const contentPrompt = `
        ...
        ${Array.isArray(structure.secciones) ? structure.secciones.map(s => `  - ${s.subtitulo}\n`).join('') : ''}
  ...
  try {
    ...
    // Búsqueda de imagen usando imageQuery de la estructura
    ...
    return {
      textContent,     // Texto largo
      structure,       // { titulo, introduccion, secciones, conclusion, imageQuery }
      imageUrl,        // URL de imagen (o null)
      userName,
      today,
      topic,
    };
...
/* ============================================
   FUNCIÓN 4: Chat público (landing / homepage)
   ============================================ */
